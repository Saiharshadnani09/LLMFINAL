  ...
  return (
    <div className="p-3 border rounded bg-white flex items-center justify-between">
      <div>
        ...
        <p className="text-xs text-gray-500">Submitted: {new Date(result.createdAt).toLocaleString()}</p>
      </div>
      <button onClick={() => onOpen(result)} className="px-3 py-1 border rounded bg-blue-600 text-white">View</button>
    </div>
  );
}
...
function ResultDetailModal({ open, onClose, result }) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-white rounded shadow-lg max-w-3xl w-full p-5 max-h-[80vh] overflow-auto">
...
function AdminStudentProfile() {
  ...
  const filtered = useMemo(() => {
    ...
    return results.filter((r) => (r.examType || r?.examId?.examType) === mode);
  }, [results, mode]);
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link className="text-blue-700 underline" to="/admin/students">Back to Students</Link>
      </div>
      ...
      <div className="mb-4 flex gap-2">
        {[
          ...
          <button key={key} className={`px-3 py-1 border rounded ${mode === key ? "bg-blue-600 text-white" : "bg-white"}`} onClick={() => setMode(key)}>
            {label}
          </button>
        ))}
      </div>
